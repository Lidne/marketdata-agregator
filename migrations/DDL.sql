-- Active: 1771158062110@@127.0.0.1@5432@market_data
-- Компании
...
CREATE TABLE instruments (
    ...
    logo_url VARCHAR,
    brand_uid UUID REFERENCES brands (uid) ON DELETE RESTRICT,
...
CREATE TABLE futures (
    ...
    min_price_increment_amount NUMERIC(20, 8),
...
ALTER TABLE trades
ADD CONSTRAINT fk_trades_instruments FOREIGN KEY (instrument_uid) REFERENCES instruments (uid) ON DELETE CASCADE;

SELECT create_hypertable (
        'trades', 'traded_at', if_not_exists => TRUE
    );

CREATE INDEX IF NOT EXISTS idx_trades_instrument_time ON trades (instrument_uid, traded_at);
...
ALTER TABLE candles
ADD CONSTRAINT fk_candles_instruments FOREIGN KEY (instrument_uid) REFERENCES instruments (uid) ON DELETE CASCADE;

SELECT create_hypertable (
        'candles', 'period_start', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE
    );

-- Уникальность свечи в рамках инструмента + таймфрейма + начала интервала
CREATE UNIQUE INDEX IF NOT EXISTS ux_candles_natural ON candles (
    instrument_uid,
    interval_seconds,
    period_start
);

CREATE INDEX IF NOT EXISTS idx_candles_instrument_time ON candles (instrument_uid, period_start);
...
ADD CONSTRAINT fk_obs_instruments FOREIGN KEY (instrument_uid) REFERENCES instruments (uid) ON DELETE CASCADE;

SELECT create_hypertable (
        'order_book_snapshots', 'snapshot_at', if_not_exists => TRUE
    );

CREATE INDEX IF NOT EXISTS idx_obs_instrument_time ON order_book_snapshots (instrument_uid, snapshot_at);

-- предотвращает дубли одинакового времени/глубины по инструменту
CREATE UNIQUE INDEX IF NOT EXISTS ux_obs_natural ON order_book_snapshots (
    instrument_uid,
    snapshot_at,
    depth
);